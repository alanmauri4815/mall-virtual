...
-- Seed the mall administrator role when this Auth user already exists.
insert into public.user_profiles (auth_user_id, email, display_name, role)
select id, email, coalesce(raw_user_meta_data->>'nickname', 'Administrador Mall'), 'admin'
from auth.users
where lower(email) = 'alanmauri4815@gmail.com'
...
grant execute on function public.resolve_member_login_email(text) to anon, authenticated;
...
grant execute on function public.resolve_tenant_login_email(text) to anon, authenticated;
...
using (
    exists (
        ...
        where s.local_code = store_products.local_code
...
using (
    exists (
        select 1 from public.stores s
        where s.local_code = contact_messages.store_id
        and s.owner_id = auth.uid()
    )
);
