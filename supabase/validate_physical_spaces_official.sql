...
missing_in_db as (
    ...
    left join public.physical_spaces ps
      on ps.physical_space_id = oc.physical_space_id
    where ps.physical_space_id is null
),
unexpected_in_db as (
    select ps.physical_space_id, ps.display_code
    from public.physical_spaces ps
    left join official_codes oc
      on oc.physical_space_id = ps.physical_space_id
    where oc.physical_space_id is null
),
mismatched_codes as (
    select
        ps.physical_space_id,
        ps.display_code as db_display_code,
        oc.display_code as official_display_code
    from public.physical_spaces ps
    join official_codes oc
      on oc.physical_space_id = ps.physical_space_id
    where coalesce(trim(ps.display_code), '') <> coalesce(trim(oc.display_code), '')
),
duplicate_display_codes as (
    select display_code, count(*) as total_spaces
    from public.physical_spaces
    group by display_code
    having count(*) > 1
)
select 'missing_in_db' as check_name, count(*)::text as result from missing_in_db
union all
select 'unexpected_in_db', count(*)::text from unexpected_in_db
union all
select 'mismatched_codes', count(*)::text from mismatched_codes
union all
select 'duplicate_display_codes', count(*)::text from duplicate_display_codes;

-- Detalle opcional:
-- select * from missing_in_db order by physical_space_id;
-- select * from unexpected_in_db order by physical_space_id;
-- select * from mismatched_codes order by physical_space_id;
-- select * from duplicate_display_codes order by display_code;
