        ...
        function createAvatar(nickname, styleCode = "1") {
            ...
            const clothMat = new THREE.MeshStandardMaterial({ color: clothColor, roughness: 0.82 });
            const accentMat = new THREE.MeshStandardMaterial({ color: accentColor, roughness: 0.7, metalness: styleCode === "1" ? 0.15 : 0.02 });
            ...
            const createLimb = (radius, height, material, name) => {
                ...
                const upperGeom = new THREE.CylinderGeometry(radius, radius * 0.92, upperH, 8);
                upperGeom.translate(0, -upperH / 2, 0);
                const upper = addTagged(root, new THREE.Mesh(upperGeom, material), `${name}_upper`);

                const joint = addTagged(upper, new THREE.Mesh(new THREE.SphereGeometry(radius * 1.08, 8, 8), material), `${name}_joint`);
                joint.position.set(0, -upperH, 0);

                const lowerGeom = new THREE.CylinderGeometry(radius * 0.92, radius * 0.78, lowerH, 8);
            ...
            const shoulders = addTagged(group, new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.14, 0.2), accentMat), "shoulders");
            shoulders.position.set(0, 1.42, 0);

            const torso = addTagged(group, new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.17, 0.68, 8), clothMat), "torso");
            torso.position.set(0, 1.14, 0.01);

            const waist = addTagged(group, new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.19, 0.18, 8), accentMat), "waist");
            waist.position.set(0, 0.81, 0);

            if (styleCode === "1") {
                const tie = addTagged(group, new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.28, 0.03), accentMat), "tie");
                tie.position.set(0, 1.08, 0.17);
            } else if (styleCode === "2") {
                const stripe = addTagged(group, new THREE.Mesh(new THREE.BoxGeometry(0.36, 0.08, 0.3), accentMat), "stripe");
                stripe.position.set(0, 1.16, 0.02);
            } else {
                const strap = addTagged(group, new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.65, 0.05), accentMat), "strap");
                strap.position.set(0.08, 1.12, 0.14);
                strap.rotation.z = -0.45;
            }

            const neck = addTagged(group, new THREE.Mesh(new THREE.CylinderGeometry(0.065, 0.07, 0.1, 8), skinMat), "neck");
            neck.position.set(0, 1.5, 0);

            const headPivot = new THREE.Group();
            headPivot.name = "headPivot";
            headPivot.position.set(0, 1.69, 0);
            group.add(headPivot);

            const head = addTagged(headPivot, new THREE.Mesh(new THREE.SphereGeometry(0.2, 16, 16), skinMat), "head");
            head.scale.set(0.94, 1.08, 0.92);

            const nose = addTagged(headPivot, new THREE.Mesh(new THREE.ConeGeometry(0.028, 0.08, 8), skinMat), "nose");
            nose.position.set(0, -0.01, 0.17);
            nose.rotation.x = Math.PI / 2;

            const eyeL = new THREE.Group();
            eyeL.name = "eyeL";
            eyeL.position.set(-0.075, 0.05, 0.16);
            const eyeballL = addTagged(eyeL, new THREE.Mesh(new THREE.SphereGeometry(0.038, 8, 8), whiteMat), "eyeL_ball");
            const pupilL = addTagged(eyeL, new THREE.Mesh(new THREE.SphereGeometry(0.018, 8, 8), blackMat), "eyeL_pupil");
            pupilL.position.z = 0.03;
            headPivot.add(eyeL);

            const eyeR = new THREE.Group();
            eyeR.name = "eyeR";
            eyeR.position.set(0.075, 0.05, 0.16);
            const eyeballR = addTagged(eyeR, new THREE.Mesh(new THREE.SphereGeometry(0.038, 8, 8), whiteMat), "eyeR_ball");
            const pupilR = addTagged(eyeR, new THREE.Mesh(new THREE.SphereGeometry(0.018, 8, 8), blackMat), "eyeR_pupil");
            pupilR.position.z = 0.03;
            headPivot.add(eyeR);

            let hairGeom;
            if (styleCode === "1") hairGeom = new THREE.SphereGeometry(0.21, 16, 16, 0, Math.PI * 2, 0, Math.PI / 1.75);
            else if (styleCode === "2") hairGeom = new THREE.BoxGeometry(0.26, 0.1, 0.42);
            else hairGeom = new THREE.SphereGeometry(0.22, 16, 16);

            const hair = addTagged(headPivot, new THREE.Mesh(hairGeom, hairMat), "hair");
            hair.position.set(0, 0.06, styleCode === "2" ? 0.05 : 0);

            if (styleCode === "2") {
                const visor = addTagged(headPivot, new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.025, 0.12), hairMat), "visor");
                visor.position.set(0, 0.02, 0.23);
            }

            const armL = createLimb(0.058, 0.63, clothMat, "armL");
            armL.root.position.set(-0.29, 1.42, 0.01);
            armL.root.rotation.z = 0.12;
            const handL = addTagged(armL.lower, new THREE.Mesh(new THREE.SphereGeometry(0.055, 8, 8), skinMat), "handL");
            handL.position.set(0, -0.34, 0);
            group.add(armL.root);

            const armR = createLimb(0.058, 0.63, clothMat, "armR");
            armR.root.position.set(0.29, 1.42, 0.01);
            armR.root.rotation.z = -0.12;
            const handR = addTagged(armR.lower, new THREE.Mesh(new THREE.SphereGeometry(0.055, 8, 8), skinMat), "handR");
            handR.position.set(0, -0.34, 0);
            group.add(armR.root);

            const legL = createLimb(0.078, 0.8, accentMat, "legL");
            legL.root.position.set(-0.11, 0.8, 0);
            const footL = addTagged(legL.lower, new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.08, 0.3), blackMat), "footL");
            footL.position.set(0, -0.42, 0.08);
            group.add(legL.root);

            const legR = createLimb(0.078, 0.8, accentMat, "legR");
            legR.root.position.set(0.11, 0.8, 0);
            const footR = addTagged(legR.lower, new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.08, 0.3), blackMat), "footR");
        ...
        function applyAvatarPose(actor, movementAmount = 0, nowMs = performance.now()) {
            ...
            if (moving) {
                actor.motionPhase += Math.max(0.08, movementAmount * 22);
            }

            const walkPhase = actor.motionPhase;
            const idlePhase = nowMs * 0.0018 + actor.idlePhase;
            const breath = Math.sin(idlePhase) * 0.018;
            const bob = moving ? Math.abs(Math.sin(walkPhase)) * 0.075 : breath;
            const sway = moving ? Math.sin(walkPhase) : Math.sin(idlePhase * 0.75) * 0.12;
            const twist = moving ? Math.sin(walkPhase) * 0.08 : Math.sin(idlePhase * 0.6) * 0.02;
            const stride = moving ? Math.min(1, 0.35 + movementAmount * 24) : 0;
            ...
            rig.shoulders.rotation.y = twist * 0.75;
            rig.shoulders.rotation.z = -sway * 0.04;
            rig.torso.rotation.x = moving ? -0.04 + Math.abs(Math.sin(walkPhase)) * 0.03 : 0.01 + breath * 0.25;
            rig.torso.rotation.y = twist;
            rig.torso.rotation.z = sway * 0.05;
            rig.waist.rotation.y = -twist * 0.5;
            rig.waist.rotation.z = sway * 0.03;
            rig.headPivot.rotation.x = moving ? -Math.abs(Math.sin(walkPhase + 0.5)) * 0.035 : breath * 0.3;
            rig.headPivot.rotation.y = -twist * 0.35;
            rig.headPivot.rotation.z = -sway * 0.03;

            const armSwingL = moving ? Math.sin(walkPhase + Math.PI) * 0.82 * stride : -0.12 + breath * 0.4;
            const armSwingR = moving ? Math.sin(walkPhase) * 0.82 * stride : 0.08 - breath * 0.4;
            const legSwingL = moving ? Math.sin(walkPhase) * 0.72 * stride : -0.02;
            const legSwingR = moving ? Math.sin(walkPhase + Math.PI) * 0.72 * stride : 0.02;

            rig.armL.root.rotation.z = base.armLRotZ - sway * 0.08;
            rig.armR.root.rotation.z = base.armRRotZ + sway * 0.08;
            rig.armL.root.rotation.x = armSwingL;
            rig.armR.root.rotation.x = armSwingR;
            rig.armL.lower.rotation.x = moving ? (armSwingL > 0 ? -armSwingL * 0.45 : -0.04) : -0.16;
            rig.armR.lower.rotation.x = moving ? (armSwingR > 0 ? -armSwingR * 0.45 : -0.04) : -0.12;

            rig.legL.root.rotation.x = legSwingL;
            rig.legR.root.rotation.x = legSwingR;
            rig.legL.root.rotation.z = sway * 0.04;
            rig.legR.root.rotation.z = -sway * 0.04;
            rig.legL.lower.rotation.x = moving ? Math.max(0, -legSwingL) * 0.95 : 0.05;
            rig.legR.lower.rotation.x = moving ? Math.max(0, -legSwingR) * 0.95 : 0.02;
            rig.footL.rotation.x = moving ? Math.max(0, legSwingL) * -0.4 : 0;
            rig.footR.rotation.x = moving ? Math.max(0, legSwingR) * -0.4 : 0;
        }
        ...
        function updateOtherPlayers() {
            Object.values(otherPlayers).forEach(p => {
                if (p.mesh) {
                    ...
                    while (rotDiff > Math.PI) rotDiff -= Math.PI * 2;
                    p.mesh.rotation.y += rotDiff * 0.16;

                    const stepDistance = prevPos.distanceTo(p.mesh.position);
                    applyAvatarPose(p, stepDistance, performance.now());
        ...
        function updateNPCs() {
            ...
            npcs.forEach(npc => {
                ...
                applyAvatarPose(npc, movedAmount, performance.now());
